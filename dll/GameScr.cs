...
public class GameScr : mScreen, IChatable
{
    ...
    private GUIStyle guiHPMain;

    private GUIStyle guiHPBorder;
    ...
    public GameScr()
    {
        ...
        guiHPMain = new GUIStyle(GUI.skin.label)
        {
            fontSize = (int)8 * mGraphics.zoomLevel,
            fontStyle = FontStyle.Bold,
            alignment = TextAnchor.UpperLeft,
        };
        guiHPMain.normal.textColor = Color.black;

        guiHPBorder = new GUIStyle(GUI.skin.label)
        {
            fontSize = (int)8 * mGraphics.zoomLevel,
            fontStyle = FontStyle.Bold,
            alignment = TextAnchor.UpperLeft,
        };
        guiHPBorder.normal.textColor = Color.white;
            ...
            if (GameCanvas.keyAsciiPress != 0)
            {
                if (mobCapcha == null)
                {
                    if (TField.isQwerty)
                    {
                        ...
                        {
                            MyMain.updateKey(GameCanvas.keyAsciiPress);
                        }
                    }
    ...
    private void paintImageBar(mGraphics g, bool isLeft, Char c)
    {
        if (c != null)
        {
            ...
            if (c == global::Char.myCharz())
            {
                ...
                int num6 = 4;
                int num7 = 19;
                string moneys = NinjaUtil.getMoneys((long)global::Char.myCharz().cHP);
                string moneys2 = NinjaUtil.getMoneys((long)global::Char.myCharz().cMP);
                g.setColor(new Color(0.2f, 0.2f, 0.2f, 0.6f));
                if (mGraphics.zoomLevel > 1)
                {
                    guiHPMain.fontSize = 8 * mGraphics.zoomLevel;
                    guiHPBorder.fontSize = 8 * mGraphics.zoomLevel;
                    StringHandle.drawStringBd(guiHPMain, g, moneys, (float)num5, (float)num6, guiHPBorder);
                    guiHPMain.fontSize = 5 * mGraphics.zoomLevel;
                    guiHPBorder.fontSize = 5 * mGraphics.zoomLevel;
                    StringHandle.drawStringBd(guiHPMain, g, moneys2, (float)num5, (float)num7, guiHPBorder);
                    return;
                }
                g.fillRect(num5 - 1, num6 + 1, mFont.tahoma_7b_yellow.getWidth(moneys), mFont.tahoma_7b_yellow.getHeight() - 2);
                mFont.tahoma_7b_yellow.drawString(g, moneys, num5, num6, mFont.LEFT);
                g.fillRect(num5 - 1, num7 + 1, mFont.tahoma_7_yellow.getWidth(moneys2), mFont.tahoma_7_yellow.getHeight() - 2);
                mFont.tahoma_7_yellow.drawString(g, moneys2, num5, num7, mFont.LEFT);
            }
        }
    }
            ...
            if (isHaveSelectSkill)
            {
                ...
                for (int i = 0; i < num4; i++)
                {
                    if (Main.isPC)
                    {
                        ...
                        int num5 = -13;
                        StringHandle.paint(mFont.tahoma_7_white, g, array2[i], xSkill + xS[i] + 14, GameScr.yS[i] + num5 + 1, mFont.CENTER, mFont.tahoma_7b_dark, "border", mGraphics.zoomLevel);
                    }
                    ...
                    long coolDown = (long)skill.coolDown - mSystem.currentTimeMillis() + skill.lastTimeUseThisSkill;
                    StringHandle.paint(mFont.tahoma_7_white, g, (coolDown < 0L) ? string.Empty : ((coolDown > 1000L) ? (coolDown / 1000L).ToString() : TimeSpan.FromMilliseconds((double)coolDown).TotalSeconds.ToString("F2")), GameScr.xSkill + GameScr.xS[i] + 14, GameScr.yS[i] + 7, mFont.CENTER, mFont.tahoma_7, "border", mGraphics.zoomLevel);
                }
            }
            paintGamePad(g);
        }
