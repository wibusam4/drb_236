    ...
    public class Boss
    {
        ...
        static void PaintListBosses(mGraphics g)
        {
            ...
            for (int i = start - offset; i < listBosses.Count - offset; i++)
            {
                ...
                int length = StringHandle.getWidth(styles[i - start + offset], $"{i + 1}. {boss}");
        ...
        static void PaintRect(mGraphics g)
        {
            ...
            titleWidth = StringHandle.getWidth(style, LIST_BOSS);
            g.setColor(new Color(.2f, .2f, .2f, .7f));
        ...
        {
            if (lastBoss != -1 && mSystem.currentTimeMillis() - GameCanvas.lastTimePress > 200)
                lastBoss = -1;
            if (!isEnabled)
                return;
            ...
            for (int i = start - offset; i < listBosses.Count - offset; i++)
            {
                if (GameCanvas.isPointerHoldIn(GameCanvas.w - (x + offsetX) - maxLength, y + 1 + distanceBetweenLines * (i - start + offset), maxLength, 7))
                {
                    ...
                    if (GameCanvas.isPointerClick)
                    {
                        ...
                        {
                            if (lastBoss == i && mSystem.currentTimeMillis() - GameCanvas.lastTimePress <= 200)
                            {
