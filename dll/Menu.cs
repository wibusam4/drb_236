...
public class Menu
{
    ...
    public void paintMenu(mGraphics g)
    {
        ...
        for (int i = 0; i < menuItems.size(); i++)
        {
            ...
            {
                if (array[j].Contains("Đang Bật"))
                {
                    StringHandle.drawStringBd(mFont.tahoma_7b_dark, g, array[j], menuX + i * menuW + menuW / 2, num + j * 14, 2, mFont.tahoma_7b_white);
                }
                else if (array[j].Contains("Đang Tắt"))
                {
                    StringHandle.drawStringBd(mFont.tahoma_7b_white, g, array[j], menuX + i * menuW + menuW / 2, num + j * 14, 2, mFont.tahoma_7b_dark);
                }
                else if (i == menuSelectedItem)
                {
                ...
                StringHandle.paint(mFont.tahoma_7, g, i.ToString(), this.menuX + i * this.menuW + 7, Menu.menuTemY[i] + 3, 0, mFont.tahoma_7b_white, "border", mGraphics.zoomLevel);
            }
        }
        g.translate(-g.getTranslateX(), -g.getTranslateY());
    }
